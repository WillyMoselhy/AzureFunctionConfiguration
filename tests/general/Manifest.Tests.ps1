Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\AzureFunctionConfiguration").Path
	$manifest = ((Get-Content "$moduleRoot\AzureFunctionConfiguration.psd1") -join "`n") | Invoke-Expression
