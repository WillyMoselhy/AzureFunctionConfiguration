...
#>
param (
	$WorkingDirectory = "$($env:SYSTEM_DEFAULTWORKINGDIRECTORY)\_AzureFunctionConfiguration",
...
$moduleName = 'AzureFunctionConfiguration'
...
"@
	...
	if (AzureFunctionConfiguration-Path -Path "$($WorkingDirectory)\azFunctionResources\functionOverride\$($functionSourceFile.BaseName).json")
	{
	...
	if (AzureFunctionConfiguration-Path -Path "$($WorkingDirectory)\azFunctionResources\functionOverride\$($condensedName).json")
	{
