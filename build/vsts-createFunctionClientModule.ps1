...
$parentModule = 'AzureFunctionConfiguration'
if (-not $ModuleName) { $ModuleName = 'AzureFunctionConfiguration.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AzureFunctionConfiguration'
...
foreach ($functionSourceFile in (Get-ChildItem -Path "$($publishRoot)\functions" -Recurse -Filter '*.ps1'))
{
	...
	$override = @{ }
	if (AzureFunctionConfiguration-Path -Path "$($WorkingDirectory)\azFunctionResources\functionOverride\$($functionSourceFile.BaseName).psd1")
	{
	...
	if (AzureFunctionConfiguration-Path -Path "$($WorkingDirectory)\azFunctionResources\functionOverride\$($condensedName).psd1")
	{
	...
	if (AzureFunctionConfiguration-Path -Path "$($WorkingDirectory)\azFunctionResources\functionOverride\$($functionSourceFile.BaseName).ps1")
	{
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AzureFunctionConfiguration.psd1"
$prereqHash = @{
